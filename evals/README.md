...
若某一项收到 HTTP 400，可用 `--scenarios` 只重测该项，并给报告新的文件名。例如：

```powershell
.\.venv\Scripts\python.exe scripts/evaluate_agent.py --live --extended --scenarios sibling-isolation --output evals/sibling-diagnostic.local.json
```

诊断记录服务商 HTTP 状态、错误类型/代码/参数、脱敏的短错误信息和请求 ID；字段缺失显示 `null`。若服务商错误正文包含长请求或提问，脚本会省略它。首次服务商故障会停止剩余调用，避免重复计费。报告仍可能包含正常模型答案，分享前请检查。
