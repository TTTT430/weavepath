...
保存连接后，可直接在发送按钮左侧切换模型与推理强度。输入框左侧的 `＋` 支持单文件不超过 20 MiB 的 UTF-8 文本、代码和结构化数据附件，最多同时选择 5 个。完整文本独立保存在本机 SQLite 的路线附件记录中；消息只保存附件 ID、名称、MIME、大小和用户问题。首次绑定时 Core 会确定性选取并固化模型上下文摘录，后续继续对话、分支和 Agent Run 都复用该内容，从而兼顾大文件与 KV-cache 前缀稳定性。附件不能跨 workflow 或路线伪造引用。PDF、图片、二进制解析、语义检索和全文搜索仍未实现，不会伪装成“已支持”。
