...
2026-09-09 的当前统一本机自动化基线包括后端 174 项测试、Python compileall、前端 147 项测试、TypeScript typecheck 和 production build；既有双层画布/Route-to-Agent Run/Engineering Lab 浏览器验收仍保留。Runtime v2 P0 新增的审批、取消、重试、cache-aware context 和 usage 可观测性目前完成自动化验证，尚未宣称完成新的真实供应商浏览器 E2E。schema v5 加入 Engineering Lab preview，schema v6 将精确轮次分支收纳为顶层对话内部的 Turn Tree 路线，schema v7 为自动分支标题增加持久化来源标记；Runtime 表使用独立 migration marker，不改变 graph schema v7。Local Chat 的 SSE/取消/回答重试已完成；普通回复也会持久化实际用时和供应商返回的 OpenAI/DeepSeek 缓存 usage，并在回复下方的可展开详情中显示，供应商未报告时明确标记不可用。任意 shell、写文件、网络工具、自动 evaluator/scorer、多 Agent、跨进程 run lease，以及正式 Codex/Claude adapter 仍未包含。
...
保存连接后，可直接在发送按钮左侧切换模型与推理强度。输入框左侧的 `＋` 支持单文件不超过 50 MiB 的 UTF-8 文本/代码/结构化数据、PDF、DOCX、XLSX 和 PPTX，每条消息最多选择 5 个。原始字节按 SHA-256 独立保存在本机 `files/objects`，SQLite 只保留元数据、解析状态、派生分块和已固化的模型上下文；消息只保存附件 ID、名称、MIME、大小和用户问题。首次绑定时 Core 会按问题确定性选取分块并固化来源，后续继续对话、分支和 Agent Run 都复用该内容，从而兼顾大文件、可追溯引用与 KV-cache 前缀稳定性。附件不能跨 workflow 或记忆路线伪造引用；路线文件面板只展示当前路线及其父路线可见文件。图片会保留原件并明确显示 OCR 尚未配置，服务端 embedding/向量检索和全文索引仍未实现，不会伪装成“已支持”。
