...
保存连接后，可直接在发送按钮左侧切换模型与推理强度。输入框左侧的 `＋` 支持单文件不超过 50 MiB 的 UTF-8 文本/代码/结构化数据、PDF、DOCX、XLSX 和 PPTX，每条消息最多选择 5 个。原始字节按 SHA-256 独立保存在本机 `files/objects`，SQLite 只保留元数据、解析状态、派生分块、FTS5 trigram 全文索引和已固化的模型上下文；消息只保存附件 ID、名称、MIME、大小和用户问题。首次绑定时 Core 会按问题确定性选取分块并固化来源，后续继续对话、分支和 Agent Run 都复用该内容，从而兼顾大文件、可追溯引用与 KV-cache 前缀稳定性。附件不能跨 workflow 或记忆路线伪造引用；路线文件面板只展示当前路线及其父路线可见文件，并显示索引覆盖状态。回复详情中的文件证据只代表本次请求实际绑定的分块，可点击回到原文件定位。图片会保留原件并明确显示 OCR 尚未配置；embedding/向量语义检索和自动路线级 RAG 尚未实现，不会伪装成“已支持”。
