...
保存连接后，可直接在发送按钮左侧切换模型与推理强度。输入框左侧的 `＋` 支持文本、代码和结构化数据附件；当前实现会把完整文本安全绑定到该条用户消息，因此适合中小型上下文文件。PDF、图片、二进制文件和超过 2 MiB 的长文档尚未伪装成“已支持”，后续将使用服务端文件存储、解析和分块检索，而不是无限放大模型请求。
