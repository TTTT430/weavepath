...
import type{AISettingsInput,ConnectionDiagnostics,NetworkMode,ReasoningEffort}from'../domain/types';
...
export function ModelSettingsDialog({onClose,onSaved}:{onClose:()=>void;onSaved:()=>void}){
 ...
 const{t,locale,setLocale}=useI18n();const[provider,setProvider]=useState<Preset>(draft.provider&&draft.provider in presets?draft.provider:'custom'),[baseUrl,setBaseUrl]=useState(draft.baseUrl||''),[model,setModel]=useState(draft.model||''),[apiKey,setApiKey]=useState(''),[persist,setPersist]=useState(draft.persist??false),[persistApiKey,setPersistApiKey]=useState(draft.persistApiKey??false),[networkMode,setNetworkMode]=useState<NetworkMode>(draft.networkMode||'auto'),[reasoningEffort,setReasoningEffort]=useState<ReasoningEffort|null>(null),[hasKey,setHasKey]=useState(false),[secureStorageAvailable,setSecureStorageAvailable]=useState(false),[models,setModels]=useState<string[]>([]),[busy,setBusy]=useState(false),[notice,setNotice]=useState<Notice|null>(null),[confirmReset,setConfirmReset]=useState(false),[loaded,setLoaded]=useState(false);
 useEffect(()=>{api.aiSettings().then(x=>{if(!draft.baseUrl)setBaseUrl(x.baseUrl||'');if(!draft.model)setModel(x.model||'');if(draft.persist===undefined)setPersist(x.persistence==='local');if(draft.persistApiKey===undefined)setPersistApiKey(Boolean(x.apiKeyPersisted));if(!draft.networkMode)setNetworkMode(x.networkMode||'auto');setReasoningEffort(x.reasoningEffort||null);setHasKey(x.hasApiKey);setSecureStorageAvailable(Boolean(x.secureKeyStorageAvailable));if(x.credentialError)setNotice({message:t('secureStorageError'),tone:'error'});setLoaded(true)}).catch(e=>{setNotice({message:e instanceof Error?e.message:String(e),tone:'error'});setLoaded(true)})},[]);
 ...
 function body():AISettingsInput{return{baseUrl:baseUrl.trim(),model:model.trim(),...(apiKey?{apiKey}:{}),persistence:persist?'local':'memory',persistApiKey,networkMode,reasoningEffort}}
