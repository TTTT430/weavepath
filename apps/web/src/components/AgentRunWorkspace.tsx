     ...
     <div className="agent-runs-layout">
      <div className="run-list">{!runs.length&&!listLoading&&<p>{t('noRuns')}</p>}{runs.map(run=><button type="button" key={run.runId} className={String(selected?.runId)===String(run.runId)?'current':''} onClick={()=>void inspect(run,key)}><strong>{run.objective}</strong><span className={`run-status ${run.status}`}>{status(run)}</span><small>{t('attempt')}: {run.attemptNumber??1} · {t('revision')}: {run.inputContentRevision}</small>{run.errorCode&&<em>{t('errorCode')}: {run.errorCode}</em>}</button>)}</div>
       ...
       <dl className="run-provenance compact">
        ...
        {selected.executionPhase&&<div><dt>{locale==='zh-CN'?'执行阶段':'Execution phase'}</dt><dd>{selected.executionPhase}</dd></div>}
      ...
      <small>{t('concreteRoute')}</small>
	      ...
	      {selected.compactionPlan&&<section className="run-compaction-plan"><div><h4>{t('contextCompaction')}</h4><span>{Math.round(selected.compactionPlan.compressionRatio*100)}%</span></div><p>{t('contextCompacted')}</p><dl><div><dt>{t('originalMessages')}</dt><dd>{selected.compactionPlan.originalMessages.toLocaleString(locale)}</dd></div><div><dt>{t('compactedMessages')}</dt><dd>{selected.compactionPlan.compactedMessages.toLocaleString(locale)}</dd></div><div><dt>{t('retainedMessages')}</dt><dd>{selected.compactionPlan.retainedMessages.toLocaleString(locale)}</dd></div></dl><small>{selected.compactionPlan.routeInstanceIds.join(' → ')}</small></section>}
	      {selected.retrievalPlan&&<section className="run-retrieval-plan"><div><h4>{t('retrievalPlan')}</h4><span>{selected.retrievalPlan.selectedChunks} / {selected.retrievalPlan.candidateCount}</span></div><p>{t('autoRetrieved')} · {selected.retrievalPlan.selectedCharacters.toLocaleString(locale)} {t('selectedCharacters')}</p><ul>{selected.retrievalPlan.sources.map((source,index)=><li key={`${source.attachmentId}-${source.chunkOrdinal}-${index}`}><strong>{source.name} · {source.locator}</strong><small>{source.routeTitle}{source.matchedTerms?.length?` · ${t('matchedTerms')}: ${source.matchedTerms.join(', ')}`:''}</small></li>)}</ul><small>{selected.retrievalPlan.truncated?t('retrievalTruncated'):t('retrievalComplete')}</small></section>}
