...
describe('MarkdownMessage',()=>{
 ...
 it('keeps a semantic table inside its horizontal scroll container',()=>{const{container}=render(<MarkdownMessage content={'| Dataset | Features | Benefits | Limits |\n|---|---|---|---|\n| A | Long content | Stable | None |'}/>);const scrollContainer=container.querySelector('.markdown-table-scroll');const table=screen.getByRole('table');expect(scrollContainer).toBeInTheDocument();expect(scrollContainer?.firstElementChild).toBe(table);expect(table.closest('.markdown-table-scroll')).toBe(scrollContainer);expect(screen.getAllByRole('columnheader')).toHaveLength(4);expect(screen.getAllByRole('cell')).toHaveLength(4)});
