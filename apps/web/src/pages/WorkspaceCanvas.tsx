...
export function WorkspaceCanvas({workflowId,visible=true,onContinue,onClose}:WorkspaceCanvasProps){
 ...
 async function commitPrune(){if(!graph||!node||!plan)return;setBusy(true);try{const result=await api.pruneCommit(graph.workflowId,plan.targetInstanceId||node.id,plan);setPlan(null);notifyWorkflowChanged({type:'conversation-workflow-changed',workflowId:graph.workflowId,...(result.activeInstanceId?{instanceId:result.activeInstanceId}:{})});if(layer.kind==='turn'&&plan.targetInstanceId&&plan.targetInstanceId!==node.id){await loadTurns(layer.instanceId)}else{backToWorkflow();await load()}}catch(caught){setError(caught instanceof Error?caught.message:String(caught))}finally{setBusy(false)}}
