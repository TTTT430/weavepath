...
export function WorkspaceCanvas({workflowId,visible=true,onContinue,onClose,onConversationActivated}:WorkspaceCanvasProps){
 ...
 function EditableTitle({instanceId,title}:{instanceId:string;title:string}){const editing=renamingId===instanceId,begin=()=>{setRenamingId(instanceId);setRenameDraft(title)};return <div className="inspector-title">{editing?<form onSubmit={event=>{event.preventDefault();void renameInstance(instanceId)}}><input autoFocus aria-label={t('renameConversation')} value={renameDraft} maxLength={240} onChange={event=>setRenameDraft(event.target.value)} onKeyDown={event=>{if(event.key==='Escape'){event.preventDefault();setRenamingId('');setRenameDraft('')}}}/><button type="button" onClick={()=>{setRenamingId('');setRenameDraft('')}}>{t('cancel')}</button><button className="primary" disabled={busy||!renameDraft.trim()}>{t('save')}</button></form>:<h3 className="editable-title" role="button" tabIndex={0} aria-label={`${t('renameConversation')}: ${title}`} title={t('rename')} onDoubleClick={begin} onKeyDown={event=>{if(event.key==='Enter'||event.key==='F2'||event.key===' '){event.preventDefault();begin()}}}>{title}</h3>}</div>}
