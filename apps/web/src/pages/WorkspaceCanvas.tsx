...
export function WorkspaceCanvas({workflowId,visible=true,onContinue,onClose}:WorkspaceCanvasProps){
 ...
 const selectedRouteId=layer.kind==='turn'?(selectedTurn?.routeInstanceId||turnSnapshot?.activeRouteInstanceId||layer.instanceId):'';
 ...
 const loadTurns=useCallback(async(instanceId:string)=>{
  ...
  try{
   ...
   const activeRoute=value.activeRouteInstanceId||instanceId,activeTurns=value.turns.filter(turn=>(turn.routeInstanceId||instanceId)===activeRoute);
   const next=value.turns.some(turn=>turn.id===persisted)?persisted:(activeTurns.at(-1)?.id||value.turns.at(-1)?.id||'');
 ...
 async function fork(input:{title:string;topicId?:string;initialMessage?:string}){
  ...
  try{
   const result=exactTurnBranch
    ?await api.forkChat(graph.workflowId,sourceId,{title:input.title,...(input.topicId?{topicId:input.topicId}:{}),initialMessage:input.initialMessage!,anchorMessageId:branchAnchorId!,expectedContentRevision:expected,idempotencyKey:crypto.randomUUID()})
   ...
   if(exactTurnBranch&&layer.kind==='turn'){
    ...
    if(childTurn)updateTurn(layer.instanceId,{selectedTurnId:childTurn.id});
   }else{
 ...
 return <main className="workspace-canvas-page">
  ...
  <BranchDialog open={branch} parentTitle={(branchSourceId&&turnSnapshot?.routeTitles?.[branchSourceId])||node?.title||''} busy={busy} requireMessage={branchAnchorId!==undefined} onClose={()=>{setBranch(false);setBranchAnchorId(undefined);setBranchSourceId('')}} onSubmit={input=>void fork(input)}/>
  <PruneDialog plan={plan} busy={busy} onClose={()=>setPlan(null)} onCommit={()=>void commitPrune()}/>
 </main>;

 function WorkflowInspector(){return <><h2>{t('details')}</h2>{node&&<><h3>{node.title}</h3><p>{node.summary||'—'}</p><small>{t('route')}</small><p>{routeLabel(graph!,node.id)}</p><h3>{t('routes')}</h3><div className="route-list">{routes.length?routes.map(route=><RouteChoice key={route.id} route={route} label={routeText(route)} selected={route.id===selected} onSelect={()=>selectNode(route.id)} onOpen={()=>openCanvas(route.id)}/>):<p>{t('noRoutes')}</p>}</div><div className="inspector-actions"><button className="primary" disabled={busy||node.status==='pruned'} onClick={()=>void continueConversation(node.id)}>{t('continueConversation')}</button><button disabled={busy||node.status==='pruned'} onClick={()=>openCanvas(node.id)}>{t('turnCanvas')}</button><button disabled={busy||node.status==='pruned'} onClick={()=>{setBranchSourceId('');setBranchAnchorId(undefined);setBranch(true)}}>{t('branch')}</button><button onClick={()=>toggleWorkflowCollapse(node.id)}>{canvasState.workflow.collapsedNodeIds.includes(node.id)?t('expandBranch'):t('collapseBranch')}</button><button className="danger-outline" disabled={busy||node.status==='pruned'} onClick={()=>void preparePrune()}>{t('archive')}</button></div></>}</>}

 function TurnInspector(){const instanceId=layer.kind==='turn'?layer.instanceId:'',routeId=selectedRouteId||instanceId,memoryRoute=turnSnapshot?.routeMemoryRoutes?.[routeId]||turnSnapshot?.memoryRoute||[],inheritedCount=turnSnapshot?.routeInheritedMessageCounts?.[routeId]??turnSnapshot?.inheritedMessageCount??0;return <><h2>{t('turnCanvas')}</h2><h3>{node?.title}</h3>{turnSnapshot&&<><small>{t('route')}</small><div className="route-chips">{memoryRoute.map(item=><span key={item.instanceId}>{item.title}</span>)}</div><section className="checkpoint-summary"><strong>{t('checkpointSummary')}</strong><p>{t('inheritedCount')}: {inheritedCount}</p><small>{t('canvasLocalOnly')}</small></section></>}{selectedTurn&&<TurnDetails turn={selectedTurn}/>}<div className="inspector-actions"><button className="primary" disabled={busy||node?.status==='pruned'} onClick={()=>void continueConversation(routeId)}>{t('continueConversation')}</button>{selectedTurn&&<button disabled={busy||node?.status==='pruned'} onClick={()=>{setBranchSourceId(selectedTurn.routeInstanceId||instanceId);setBranchAnchorId(selectedTurn.anchorMessageId);setBranch(true)}}>{t('branchFromTurn')}</button>}<button onClick={backToWorkflow}>{t('backToWorkflow')}</button></div></>}
