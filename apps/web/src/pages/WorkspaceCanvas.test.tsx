...
const apiMock=vi.hoisted(()=>({
 graph:vi.fn(),turns:vi.fn(),messages:vi.fn(),routes:vi.fn(),activate:vi.fn(),fork:vi.fn(),prunePlan:vi.fn(),pruneCommit:vi.fn(),
}));
...
vi.mock('../components/TurnCanvas',()=>({
 TurnCanvas:(props:any)=><div data-testid="turn-canvas" data-selected={props.selectedTurnId}>
  {props.snapshot.turns.map((turn:any)=><button type="button" key={turn.id} onClick={()=>props.onSelect(turn.id)}>{turn.userMessage.content}</button>)}
 </div>,
}));
...
const forkedSnapshot={...snapshot,instanceId:'forked',contentRevision:0,memoryRoute:[...snapshot.memoryRoute,{instanceId:'forked',title:'模块 B'}],inheritedMessageCount:8,turns:[]};
...
describe('native double canvas workspace',()=>{
 ...
 it('forks from the selected backend anchor with the snapshot revision and an idempotency key',async()=>{
  ...
  renderCanvas();await openLeafCanvas();
  await screen.findByRole('button',{name:'从此轮次创建分支'});fireEvent.click(screen.getByRole('button',{name:'从此轮次创建分支'}));
  fireEvent.change(screen.getByLabelText('对话名称'),{target:{value:'模块 B'}});fireEvent.click(screen.getByRole('button',{name:'创建并打开'}));
  await waitFor(()=>expect(apiMock.fork).toHaveBeenCalledOnce());
  expect(apiMock.fork).toHaveBeenCalledWith('wf','leaf',{title:'模块 B',anchorMessageId:77,expectedContentRevision:9,idempotencyKey:'fork-idempotency-1'});
 ...
 it('refreshes the Turn snapshot after a fork revision conflict',async()=>{
  ...
  fireEvent.change(screen.getByLabelText('对话名称'),{target:{value:'冲突分支'}});fireEvent.click(screen.getByRole('button',{name:'创建并打开'}));
