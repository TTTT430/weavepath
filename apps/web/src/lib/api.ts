...
export const api={
 ...
 renameTurn:(w:string,i:string,messageId:number,title:string,expectedRevision:number)=>request<{title:string;graphRevision:number}>(`/workflows/${enc(w)}/instances/${enc(i)}/turns/${messageId}`,{method:'PATCH',body:JSON.stringify({title,expectedRevision})}),
