...
| Local Graph Chat | Verified local preview | create/message/branch/route isolation/i18n、草稿保持、本地记录/动态继承记忆分离、Markdown、原生工作流切换、可操作 Turn Canvas、卡片快捷空分支、自动/手动标题、精确 turn 分支并回答和显式 Continue 已完成自动化验证；SSE、正式 HostAdapter 和完整 Phase 1 退出条件仍未完成 |
...
2. 为 schema v7 前向 migration runner 补齐升级矩阵、失败恢复、备份和 rollback/downgrade 发布策略。
