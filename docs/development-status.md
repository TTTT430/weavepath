...
| `backend/api/` | In progress | health/SQLite schema version 为 7；核心 `/api/v1`、OpenAI-compatible Local Chat（JSON 与 SSE 流式）、取消请求、聊天幂等、50 MiB 路线文件上传、4 MiB 分片续传/重启恢复、content-addressed object store、PDF/Office 解析、路线隔离的持久化 FTS5 trigram 全文索引与绑定检索、模型设置、Route-to-Agent Run preview、Engineering Lab preview、`GET .../turns`/`turn-tree`、可省略 prompt 的内部 `fork-chat` 和 revision-safe rename 已完成自动化验证；graph snapshot 与 legacy manifest 协议仍为 schema v1，尚无 WebSocket、认证/多租户边界和完整 application service 分层 |
| `backend/tests/` | In progress | graph/API、路线隔离、AI 设置、Agent Runtime、工程记录、工具安全、turn cursor、路线文件对象存储/解析/分块/引用隔离、分片续传/重启恢复/持久化全文搜索、崩溃恢复与孤儿清理、空分支/自动命名/手动重命名和 v1→v7 migration 已纳入套件；2026-09-09 当前完整后端套件为 175 项通过 |
| `apps/web/` | In progress | React chat/model settings、输入框快捷模型/推理强度切换、路线文件上传进度/透明续传、独立文件面板、路线全文搜索与索引覆盖状态、解析状态/来源/分块预览、回复证据到精确分块跳转、可读 Agent 执行时间线、双栏独立滚动的运行详情、审批/取消/重试与缓存 usage、原生 `WorkspaceShell`、可操作双层画布和 Engineering Lab 已实现；前端 149 项测试、typecheck/production build 和既有主路径 E2E 已建立，Runtime v2 新路径的真实 provider 浏览器 E2E、实验室写入路径和真实窄屏仍待补验 |
...
| Local AI Chat | In progress | 已实现网页/环境变量配置、模型发现、连接验证、输入框快捷模型/推理强度切换、自动/系统代理/直连与逐路线诊断、OpenAI-compatible JSON/SSE 回复、逐 token 草稿、停止生成、失败回答独立重试、请求幂等、编辑最近提问并原子重新生成、思考/内联错误状态、稳定错误码和当前路线消息写回。`＋` 当前支持 UTF-8 文本/代码/数据、PDF、DOCX、XLSX、PPTX（单文件 50 MiB、每条消息最多 5 个）：超过 4 MiB 时使用可跨重启恢复的分片上传并显示进度，原件进入本机对象存储，消息只存路线受限引用，绑定时从派生分块确定性检索并固化上下文与来源；Chat 与 Agent Runtime 共用解析链路，路线摘要不暴露附件 envelope。路线文件面板通过持久化 FTS5 trigram 索引搜索当前路线和父路线，显示索引覆盖、解析状态、引用来源和分块预览；回复详情可打开本次实际使用的精确文件证据，兄弟文件不会进入结果。图片原件可保留但 OCR 尚未配置，embedding/向量语义检索和自动路线级 RAG 尚未实现；快捷切换不触碰服务地址、密钥、安全保存和网络方式；API key 默认仅进程内存，用户显式选择时由 Windows 当前用户 DPAPI 加密保存；仍无 metabolize |
| Agent Runtime v2 P0 | Done for current slice | cache-aware 请求按 policy/tools/live route/accepted knowledge/current request 装配；checkpoint 仅审计，动态父记忆和兄弟隔离已验证。逐 model step 记录 OpenAI/DeepSeek cache usage，缺失时保持不可用；加入持久化取消、重试 lineage、审批、patch Artifact 及显式根目录只读工具。正式本机 app 已使用单进程后台队列，页面刷新不中断，启动恢复 queued/approval/cancelling；未知供应商边界的 running run 安全转 interrupted。仍无跨进程 lease，不代表 Phase 2 完成 |
...
1. **本轮 P0：路线文件检索与可追溯引用。** 已加入持久化 FTS5 trigram 索引、启动回填/删除同步、路线与兄弟隔离、索引状态，以及回复证据到精确分块的跳转。引用只代表该次请求实际绑定的文件，不把未使用文件冒充来源。
2. **下一步 P1：受预算约束的自动路线级检索。** 在不破坏稳定 prompt 前缀和兄弟隔离的前提下，为未显式选择文件的请求建立可检查的 retrieval plan；随后再评估 OCR 与可选向量语义检索。
3. **校正后的 P2：Runtime 可靠性。** 重点是模型/工具未知边界后的恢复、工具副作用幂等、审批恢复与 durable owner/lease。每次运行的时间线、费用、Token、缓存命中率和工具记录已经实现，不再作为新的 P2 重复开发。
...
- Python 3.12 统一后端套件 113 项通过，包含 schema v1→v7 migration、local-only turn projection、空路线 read model、自动/手动标题、幂等 fork-and-answer、精确 turn cursor、工程记录、stale revision 和路线隔离；
- 前端 Vitest 统一套件覆盖 WorkspaceShell 三视图切换、双击仲裁、内部 Turn Tree/Chat 同步、卡片快捷分支、Engineering Lab 的受控合并，以及画布 metadata 持久化；
- Chat/Turn Canvas 的跨 surface 生命周期事件按具体 route 与 `requestId` 隔离，仅作为刷新提示；接收方重新读取 SQLite/local snapshot，事件不承担 transcript 持久化，也不保证刷新后恢复瞬时 pending/error UI；
- Python compileall、TypeScript typecheck 与 production build 通过；
- 真实浏览器 E2E 确认工作流在当前页面原生打开、选择/双击会同步对应 Chat 路线、继承消息不冒充本地 turn、从具体 turn 创建并激活分支，以及 Continue 返回对应聊天；
...
- 当前 Python 3.12 统一后端套件 175 项通过，覆盖 run create/read/events/metrics、后台排队与 queued 重启恢复、普通回复 usage 持久化、cache-aware route context、Windows DPAPI 密钥保存、快捷模型/推理强度切换、路线受限文件上传/分片续传/对象存储/Office 解析/持久化全文搜索与引用/崩溃恢复及装配、显式网络路线、event sequence、连接重试与流重置、工具安全、adapter contract、idempotency 与 revision；前端统一套件为 149 项通过；
