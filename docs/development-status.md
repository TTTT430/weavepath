...
| `backend/api/` | In progress | health/SQLite schema version 为 7；核心 `/api/v1`、OpenAI-compatible Local Chat（JSON 与 SSE 流式）、取消请求、聊天幂等、50 MiB 路线文件上传、4 MiB 分片续传/重启恢复、content-addressed object store、PDF/Office 解析、路线限定分块搜索与绑定检索、模型设置、Route-to-Agent Run preview、Engineering Lab preview、`GET .../turns`/`turn-tree`、可省略 prompt 的内部 `fork-chat` 和 revision-safe rename 已完成自动化验证；graph snapshot 与 legacy manifest 协议仍为 schema v1，尚无 WebSocket、认证/多租户边界和完整 application service 分层 |
| `backend/tests/` | In progress | graph/API、路线隔离、AI 设置、Agent Runtime、工程记录、工具安全、turn cursor、路线文件对象存储/解析/分块/引用隔离、分片续传/重启恢复/路线搜索、崩溃恢复与孤儿清理、空分支/自动命名/手动重命名和 v1→v7 migration 已纳入套件；2026-09-09 当前完整后端套件为 174 项通过 |
| `apps/web/` | In progress | React chat/model settings、输入框快捷模型/推理强度切换、路线文件上传进度/透明续传、独立文件面板、路线内容搜索、解析状态/来源/分块预览、可读 Agent 执行时间线、双栏独立滚动的运行详情、审批/取消/重试与缓存 usage、原生 `WorkspaceShell`、可操作双层画布和 Engineering Lab 已实现；前端 147 项测试、typecheck/production build 和既有主路径 E2E 已建立，Runtime v2 新路径的真实 provider 浏览器 E2E、实验室写入路径和真实窄屏仍待补验 |
...
- 当前 Python 3.12 统一后端套件 174 项通过，覆盖 run create/read/events/metrics、后台排队与 queued 重启恢复、普通回复 usage 持久化、cache-aware route context、Windows DPAPI 密钥保存、快捷模型/推理强度切换、路线受限文件上传/分片续传/对象存储/Office 解析/分块搜索与引用/崩溃恢复及装配、显式网络路线、event sequence、连接重试与流重置、工具安全、adapter contract、idempotency 与 revision；前端统一套件为 147 项通过；
