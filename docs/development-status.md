...
| Browser window sync | Done for current slice | Chat 与 Turn Canvas 使用按 workflow、route 和 `requestId` 隔离的失效/lifecycle 提示；同页 selection 激活后以及可选 `/graph` 兼容入口都使用 `BroadcastChannel + window.opener.postMessage` 促使其他 surface 重读。这些 browser events 只触发状态提示和重新读取，SQLite/local snapshot 才是真源；它不是 WebSocket、跨设备同步或持久化的进行中 UI 状态恢复 |
